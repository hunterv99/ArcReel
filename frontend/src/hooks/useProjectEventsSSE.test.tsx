...
describe("useProjectEventsSSE", () => {
  beforeEach(() => {
    ...
    vi.spyOn(API, "getProject").mockResolvedValue({
      project: {
        ...
        characters: { hero: { description: "Dũng sĩ" } },
        clues: {},
      },
      scripts: {
        "episode_1.json": {
  ...
  it("refreshes and navigates to the focused workspace target for remote changes", async () => {
    ...
    expect(useAppStore.getState().workspaceNotifications[0]).toEqual(
      expect.objectContaining({
        text: 'AI vừa thêm mới Nhân vật "hero", nhấn để xem',
  ...
  it("defers focus when the user is editing", async () => {
    ...
    await waitFor(() => {
      expect(API.getProject).toHaveBeenCalledWith("demo");
      expect(useAppStore.getState().workspaceNotifications[0]?.target?.id).toBe("NgocBoi");
    });
  ...
  it("shows a toast without navigation for generation completion batches", async () => {
    ...
    await waitFor(() => {
      expect(API.getProject).toHaveBeenCalledWith("demo");
      expect(useAppStore.getState().toast?.text).toBe('Hình ảnh phân cảnh của Phân cảnh "E1S01" đã được tạo xong');
    });
    expect(useAppStore.getState().toast?.tone).toBe("success");
    expect(useAppStore.getState().workspaceNotifications[0]).toEqual(
      expect.objectContaining({
        text: 'Hình ảnh phân cảnh của Phân cảnh "E1S01" đã được tạo xong',
        tone: "success",
        target: null,
      }),
    );
  ...
  it("groups remote changes by type and invalidates only the touched entity keys", async () => {
    ...
    await waitFor(() => {
      expect(API.getProject).toHaveBeenCalledWith("demo");
      expect(useAppStore.getState().toast?.text).toBe('Manh mối "Ngọc Bội" đã được cập nhật');
    });
    ...
    expect(useAppStore.getState().getEntityRevision("clue:NgocBoi")).toBe(1);
    ...
    expect(useAppStore.getState().workspaceNotifications).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          text: "AI vừa thêm mới 2 Nhân vật: hero, mage, nhấn để xem",
          target: expect.objectContaining({
            type: "character",
            id: "hero",
            route: "/characters",
        ...
        expect.objectContaining({
          text: 'AI vừa cập nhật Manh mối "Ngọc Bội", nhấn để xem',
          target: expect.objectContaining({
            type: "clue",
            id: "NgocBoi",
            route: "/clues",
  ...
  it("extracts asset_fingerprints from SSE changes and updates store", async () => {
    ...
    // fingerprints phải được ghi ngay lập tức (đồng bộ) vào store, không cần đợi getProject
