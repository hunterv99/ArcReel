...
function makeTodoWriteBlock(overrides: Partial<ContentBlock> = {}): ContentBlock {
  return {
    ...
    name: "TodoWrite",
    input: {
      todos: [makeTodo("Chuẩn bị nhiệm vụ"), makeTodo("Hoàn thành nhiệm vụ", "completed")],
    },
    ...overrides,
  };
}
...
describe("ToolCallWithResult", () => {
  it("keeps successful TodoWrite calls in the compact summary mode", () => {
    ...
    expect(screen.getByText("Danh sách nhiệm vụ 1/2 hoàn thành")).toBeInTheDocument();
  ...
  it("shows the generic expandable error view for failed TodoWrite calls", () => {
    ...
    expect(screen.queryByText("Danh sách nhiệm vụ 1/2 hoàn thành")).not.toBeInTheDocument();
  });
});
