...
// ---------------------------------------------------------------------------
// SessionSelector — Bộ chọn menu xuống của phiên
// ---------------------------------------------------------------------------
...
function SessionSelector({
  ...
  const displayTitle = isDraftSession ? "Hội thoại mới" : (currentSession?.title || formatTime(currentSession?.created_at));
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <span className="max-w-24 truncate">{displayTitle || "Không có hội thoại"}</span>
            ...
            {sessions.map((session) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); if (confirm("Bạn có chắc chắn muốn xóa hội thoại này không? Thao tác này không thể hoàn tác.")) onDelete(session.id); }}
...
function formatTime(isoStr: string | undefined): string {
  if (!isoStr) return "Hội thoại mới";
  try {
...
export function AgentCopilot() {
  ...
  const inputPlaceholder = pendingQuestion
    ...
    : isRunning
      ? "Trợ lý đang tạo, bạn có thể nhấp để dừng"
      : "Nhập tin nhắn, nhập / để xem các kỹ năng khả dụng";
  ...
  const addImages = useCallback((files: File[]) => {
    ...
    for (const file of files) {
      ...
      reader.onload = (e) => {
        if (imageGenRef.current !== gen) return; // Cũ — tin nhắn đã được gửi
  ...
  const handleSend = useCallback(() => {
    ...
    imageGenRef.current += 1; // Hủy bỏ các callback FileReader đang chờ xử lý
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    // Ủy quyền cho menu lệnh slash khi đang mở
  ...
  // Theo dõi vị trí của dấu gạch chéo "/" để biết token lệnh bắt đầu từ đâu
  ...
  const handleInputChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    // Kiểm tra văn bản bên trái con trỏ: kích hoạt menu khi "/" ở đầu hoặc sau khoảng trắng/dòng mới
    ...
    // Tự động thay đổi kích thước: tăng lên đến 50vh, sau đó cuộn
    const el = e.target;
  ...
  // Lấy bộ lọc lệnh từ đầu vào (văn bản sau "/" cho đến con trỏ)
  const slashFilter = showSlashMenu && slashPosRef.current >= 0
    ? localInput.slice(slashPosRef.current + 1).split(/\s/)[0]
    : "";

  const handleSlashSelect = useCallback((cmd: string) => {
    // Thay thế token "/filter" bằng lệnh đã chọn, giữ nguyên văn bản xung quanh
    const pos = slashPosRef.current;
    if (pos >= 0) {
      const before = localInput.slice(0, pos);
      // Tìm vị trí kết thúc của token lệnh (khoảng trắng tiếp theo hoặc kết thúc chuỗi)
      ...
      <div className="border-t border-gray-800 p-3">
        ...
        >
          ...
          <button
            ...
            title={attachedImages.length >= MAX_IMAGES ? `Đính kèm tối đa ${MAX_IMAGES} hình ảnh` : "Đính kèm hình ảnh"}
          ...
          {isRunning ? (
            <button
              ...
              title="Ngắt hội thoại"
              aria-label="Ngắt hội thoại"
            >
