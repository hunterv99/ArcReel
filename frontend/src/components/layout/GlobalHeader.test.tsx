...
describe("GlobalHeader", () => {
  ...
  it("prefers the project title over the internal project name", async () => {
    ...
    expect(screen.getByText("Dự án Xin chào")).toBeInTheDocument();
  ...
  it("shows unread notification count and opens the drawer", async () => {
    ...
    useAppStore.getState().pushWorkspaceNotification({
      text: "AI vừa cập nhật manh mối 「Ngọc Bội」, nhấp để xem",
      target: {
        type: "clue",
        id: "Ngọc Bội",
        route: "/clues",
    ...
    expect(screen.getByTitle("Thông báo: 1 mục")).toBeInTheDocument();
  ...
  it("exports the current project zip via browser-native download", async () => {
    ...
    vi.spyOn(API, "requestExportToken").mockResolvedValue({
      ...
      diagnostics: {
        blocking: [],
        auto_fixed: [{ code: "current_asset_restored_from_version", message: "Sửa lỗi tham chiếu video" }],
        warnings: [],
      },
    });
    ...
    screen.getByRole("button", { name: "Xuất tệp ZIP dự án hiện tại" }).click();
    ...
    expect(useAppStore.getState().toast?.text).toContain("gói xuất chứa 1 chẩn đoán");
  });
