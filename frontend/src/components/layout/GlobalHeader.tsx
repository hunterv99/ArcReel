...
/** Kích hoạt tải xuống trình duyệt qua thẻ <a> ẩn, tránh tạo tab trống bằng window.open */
...
export function GlobalHeader({ onNavigateBack }: GlobalHeaderProps) {
  ...
  // Tải dữ liệu thống kê chi phí (tự động làm mới khi nhiệm vụ hoàn thành)
  ...
  const handleJianyingExport = async (episode: number, draftPath: string, jianyingVersion: string) => {
    ...
    try {
      ...
      useAppStore.getState().pushToast("Đã bắt đầu xuất bản thảo CapCut (Jianying), vui lòng giải nén tệp ZIP vào thư mục bản thảo của CapCut", "success");
    } catch (err) {
      useAppStore.getState().pushToast(`Xuất bản thảo CapCut thất bại: ${(err as Error).message}`, "error");
    } finally {
    ...
    try {
      ...
      if (diagnosticCount > 0) {
        ...
        useAppStore.getState().pushToast(
          `Tệp ZIP dự án đã bắt đầu tải xuống, gói xuất chứa ${diagnosticCount} chẩn đoán`,
          "warning",
        );
      } else {
        useAppStore.getState().pushToast("Tệp ZIP dự án đã bắt đầu tải xuống", "success");
      }
    } catch (err) {
      useAppStore
        .getState()
    ...
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-gray-800 bg-gray-900/80 px-4 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3">
        <div className="relative" ref={notificationAnchorRef}>
          <button
            ...
            title={`Thông báo: ${workspaceNotifications.length} mục`}
        ...
        <div className="relative" ref={exportAnchorRef}>
          <button
            ...
            title="Xuất tệp ZIP dự án hiện tại"
            aria-label="Xuất tệp ZIP dự án hiện tại"
          >
            {exportingProject ? (
      ...
      {exportDiagnostics !== null && (
        <ArchiveDiagnosticsDialog
          ...
          description="Việc xuất đã hoàn tất kiểm tra trước và tạo tệp ZIP. Các vấn đề sau đây đã được phát hiện trong gói xuất."
          sections={[
            { key: "blocking", title: "Vấn đề nghiêm trọng", tone: "border-red-400/25 bg-red-500/10 text-red-100", items: exportDiagnostics.blocking },
