...
export function CustomProviderForm({ existing, onSaved, onCancel }: CustomProviderFormProps) {
  ...
  const handleDiscover = useCallback(async () => {
    if (!baseUrl) {
      showError("Vui lòng điền Base URL trước");
      return;
    }
    if (!apiKey) {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!baseUrl.trim()) {
      showError("Vui lòng điền Base URL");
      return;
    }
    if (!isEdit && !apiKey.trim()) {
      showError("Vui lòng điền API Key");
      return;
    }
    const enabledModels = models.filter((m) => m.is_enabled);
    if (enabledModels.length === 0) {
      showError("Kích hoạt ít nhất một model");
      return;
    }
    const emptyId = enabledModels.find((m) => !m.model_id.trim());
    if (emptyId) {
      showError("Model được kích hoạt phải điền model_id");
      return;
    }
    setSaving(true);
    try {
      if (isEdit && existing) {
        // Cập nhật nguyên tử nhà cung cấp + các model trong một giao dịch duy nhất
  ...
  const urlPreview = (() => {
    ...
    // Google SDK tự động nối /v1beta, backend sẽ loại bỏ các đường dẫn phiên bản do người dùng nhập sai
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder={isEdit ? existing?.api_key_masked ?? "Để trống để giữ nguyên key hiện tại" : "Nhập API Key"}
        ...
        {models.length === 0 && (
          <div className="rounded-xl border border-dashed border-gray-700 p-4 text-center text-sm text-gray-500">
            Nhấp vào "Lấy danh sách model" để tự động tìm kiếm, hoặc
            <button
              type="button"
              onClick={addManualModel}
