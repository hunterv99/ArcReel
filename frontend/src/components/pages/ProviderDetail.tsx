    ...
    return (
      <div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              placeholder={field.is_set ? field.value_masked ?? "••••••••••" : (field.placeholder ?? "Nhập key")}
        ...
        {field.is_set && !(field.key in draft) && (
          <p className="mt-1 text-xs text-gray-600">Đã thiết lập (để trống để giữ nguyên giá trị)</p>
        )}
      </div>
    );
  }
...
export function ProviderDetail({ providerId, onSaved }: Props) {
  ...
  const handleCredentialChanged = useCallback(async () => {
    // Làm mới cấu hình trong im lặng
        ...
        <div className="mb-5 flex flex-wrap gap-1.5">
          {detail.media_types.map((t) => (
            <span key={t} className="rounded-md bg-gray-800 px-2 py-0.5 text-xs text-gray-400">
              {t === "video" ? "Video" : t === "image" ? "Hình ảnh" : t === "text" ? "Văn bản" : t}
            </span>
          ))}
        </div>
      )}
