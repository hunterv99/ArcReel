      ...
      <div className="w-full max-w-md rounded-2xl border border-amber-400/20 bg-gray-900 p-6 shadow-2xl shadow-black/40">
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            <h2 className="text-lg font-semibold text-gray-100">Phát hiện trùng ID dự án</h2>
            <p className="text-sm leading-6 text-gray-400">
              ID dự án trong gói nhập
              <span className="mx-1 rounded bg-gray-800 px-1.5 py-0.5 font-mono text-gray-200">
                {projectName}
              </span>
              đã tồn tại. Bạn có thể ghi đè dự án hiện tại hoặc tự động đổi tên để tiếp tục nhập.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3">
          <button
            ...
            disabled={importing}
            aria-label="Ghi đè dự án hiện tại"
          ...
          >
            <span>
              <span className="block font-medium">Ghi đè dự án hiện tại</span>
              <span className="mt-1 block text-xs text-red-200/80">
                Sử dụng nội dung trong gói nhập để thay thế dữ liệu của ID dự án hiện tại
              </span>
            </span>
          ...
          >
            <span>
              ...
              <span className="mt-1 block text-xs text-indigo-200/80">
                Giữ lại dự án hiện tại, dự án mới nhập sẽ tự động tạo ID nội bộ mới
              </span>
            </span>
            {importing && <Loader2 className="h-4 w-4 animate-spin" />}
          </button>
...
function ImportDiagnosticsDialogWrapper({
  ...
  return (
    <ArchiveDiagnosticsDialog
      title="Chẩn đoán nhập dữ liệu"
      description="Quá trình nhập dữ liệu đã hoàn thành kiểm tra trước. Các vấn đề dưới đây được phân loại theo mức độ nghiêm trọng, quá trình nhập sẽ không tiếp tục cho đến khi các vấn đề chặn đầu được giải quyết."
      sections={[
        { key: "blocking", title: "Vấn đề chặn đầu", tone: "border-red-400/25 bg-red-500/10 text-red-100", items: diagnostics.blocking },
    ...
    >
      ...
      {summary && summary.total > 0 && (
        <div className="text-xs text-gray-500">
          {summary.total} tập
          {summary.scripted > 0 && ` · ${summary.scripted} tập kịch bản xong`}
          {summary.in_production > 0 && ` · ${summary.in_production} tập đang làm`}
          {summary.completed > 0 && ` · ${summary.completed} tập xong`}
        </div>
      )}
    </button>
  );
    ...
    async (
      ...
      try {
        ...
        useAppStore.getState().pushToast(
          autoFixedCount > 0
            ? `Dự án "${result.project.title || result.project_name}" đã nhập, tự sửa ${autoFixedCount} lỗi`
        ...
        if (warningCount > 0) {
          ...
          useAppStore.getState().pushToast(
            `Cảnh báo nhập dữ liệu: ${warningMessages}`,
            "warning"
          );
        }
        ...
        useAppStore
          .getState()
          .pushToast(
            `Nhập dữ liệu thất bại: ${error.detail || error.message || "Nhập dữ liệu thất bại"}`
            + (blockingCount > 0 ? ` (${blockingCount} vấn đề chặn đầu` : " (0 vấn đề chặn đầu")
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-1 flex items-center gap-1 border-l border-gray-800 pl-3">
              ...
              >
                ...
                {!isConfigComplete && (
                  <span className="absolute right-0.5 top-0.5 h-2 w-2 rounded-full bg-rose-500" aria-label="Cấu hình chưa hoàn thiện" />
                )}
              </button>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-8">
        {projectsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            ...
            <p className="text-sm mt-1">Nhấp vào "Dự án mới" hoặc "Nhập ZIP" ở góc trên bên phải để bắt đầu sáng tạo</p>
          </div>
        ) : (
