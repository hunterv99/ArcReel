...
describe("StudioCanvasRouter", () => {
  ...
  it("runs character/clue callbacks and reports API failures with toast", async () => {
    ...
    vi.spyOn(API, "generateCharacter").mockResolvedValue({ success: true, task_id: "t-1", message: "Đã gửi" });
    ...
    await waitFor(() => {
      ...
      expect(useAppStore.getState().toast?.text).toContain("Đã gửi tác vụ tạo nhân vật");
