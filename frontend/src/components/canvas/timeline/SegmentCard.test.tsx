...
function makeSegment(overrides: Partial<NarrationSegment> = {}): NarrationSegment {
  return {
    ...
    duration_seconds: 4,
    segment_break: false,
    novel_text: "Ngẩng đầu trong đêm mưa.",
    characters_in_segment: ["Hero"],
    clues_in_segment: [],
    image_prompt: "Một bản vẽ phân cảnh đậm chất điện ảnh",
    video_prompt: "Ống kính tiến lên chậm rãi",
...
describe("SegmentCard", () => {
  ...
  it("shows an image fullscreen trigger and uses native video controls", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Xem toàn màn hình Ảnh phân cảnh SEG-1" }),
    ).toBeInTheDocument();
