    ...
    return (
      <div className="flex h-full flex-col gap-1.5 p-3">
        ...
        <pre className="whitespace-pre-wrap text-sm leading-relaxed text-gray-300 font-sans">
          {s.novel_text || "（Chưa có văn bản gốc）"}
        </pre>
        {noteSection}
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-3 p-3">
      ...
      <div>
        ...
        <PreviewableImageFrame src={storyboardUrl} alt={`Ảnh phân cảnh ${segmentId}`}>
          <AspectFrame ratio={normalizedRatio}>
            <ImageFlipReveal
              src={storyboardUrl}
              alt={`Ảnh phân cảnh ${segmentId}`}
              loading="lazy"
              className="h-full w-full object-cover"
              fallback={
                <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-gray-600">
                  <ImageIcon className="h-8 w-8" />
                  <span className="text-xs">Chưa có phân cảnh</span>
                </div>
              }
            />
          </AspectFrame>
