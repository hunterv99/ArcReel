...
function formatSingleNotificationText(change: ProjectChange): string {
  if (change.action === "storyboard_ready") {
    return `Hình ảnh phân cảnh của ${change.label} đã được tạo xong`;
  }
  if (change.action === "video_ready") {
    return `Video của ${change.label} đã được tạo xong`;
  }
...
function formatSingleDeferredText(change: ProjectChange): string {
  if (change.action === "storyboard_ready") {
    return `AI vừa tạo xong hình ảnh phân cảnh cho ${change.label}, nhấn để xem`;
  }
  if (change.action === "video_ready") {
    return `AI vừa tạo xong video cho ${change.label}, nhấn để xem`;
  }
  if (change.action === "created") {
    return `AI vừa thêm mới ${change.label}, nhấn để xem`;
  }
  if (change.action === "deleted") {
...
export function formatGroupedNotificationText(
  ...
  if (group.action === "storyboard_ready" || group.action === "video_ready") {
    return `Đã tạo xong ${count} ${entityLabel}: ${summary}`;
  }
  if (group.action === "created") {
    return `Đã thêm mới ${count} ${entityLabel}: ${summary}`;
  }
...
export function formatGroupedDeferredText(
  ...
  if (group.action === "storyboard_ready" || group.action === "video_ready") {
    return `AI vừa tạo xong ${count} ${entityLabel}: ${summary}, nhấn để xem`;
  }
  if (group.action === "created") {
    return `AI vừa thêm mới ${count} ${entityLabel}: ${summary}, nhấn để xem`;
  }
  if (group.action === "deleted") {
