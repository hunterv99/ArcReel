...
describe("API", () => {
  ...
  describe("request-based wrappers", () => {
    ...
    it("rejects unsupported project mode updates before sending the request", async () => {
      ...
      ).rejects.toThrow("Không hỗ trợ sửa đổi content_mode hoặc aspect_ratio sau khi tạo dự án");
      await expect(
  ...
  describe("fetch-based wrappers", () => {
    ...
    it("imports project via multipart form and preserves structured errors", async () => {
      const fetchMock = vi
        ...
        .mockResolvedValueOnce(
          mockResponse({
            ...
            statusText: "Bad Request",
            jsonData: {
              detail: "Kiểm tra gói nhập thất bại",
              errors: ["Thiếu project.json", "Thiếu scripts/episode_1.json"],
              warnings: ["Phát hiện tệp/thư mục không xác định: extra"],
              diagnostics: {
                blocking: [
                ...
                warnings: [
                  { code: "validation_warning", message: "Phát hiện tệp/thư mục không xác định: extra" },
                ],
              },
            },
          }),
      ...
      await expect(API.importProject(file)).rejects.toMatchObject({
        message: "Kiểm tra gói nhập thất bại",
        detail: "Kiểm tra gói nhập thất bại",
        errors: ["Thiếu project.json", "Thiếu scripts/episode_1.json"],
        warnings: ["Phát hiện tệp/thư mục không xác định: extra"],
        diagnostics: {
