...
class ProjectManager:
    ...
    def repair_claude_symlink(self, project_dir: Path) -> dict:
            ...
            try:
                if is_windows:
                    if target_abs.is_dir():
                        ...
                        # Use hardlink for file (no admin required)
                        os.link(target_abs, link_path)
                else:
                    link_path.symlink_to(target_rel)
                return True
            except Exception as e:
                logger.warning("无法为项目 %s 创建 %s 链接: %s", project_dir.name, name, e)
                return False

        def _remove_broken_link(link_path: Path):
            try:
                # Need to handle Junctions specifically on Windows which are treated as directories
                if link_path.is_dir() and platform.system() == "Windows":
                    os.rmdir(link_path)
                else:
                    link_path.unlink()
            except OSError as e:
        ...
        for name, target_source in SYMLINKS.items():
            ...
            # Windows junction handling: is_symlink() returns False for junctions, 
            # so we check if it exists but points to nowhere by trying to resolve?
            # Or simpler: if it exists, assume it's good unless it's a known broken symlink
            
            if is_broken_link:
                # 损坏的软连接
                _remove_broken_link(symlink_path)
